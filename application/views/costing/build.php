<div class="">
	<div class="card-body">
		<div class="row">
				<button class="btn btn-md btn-primary pull-left" onclick="costing_back_to_menu_costing()">
					<i class="fa fa-reply fa-md"></i> Back
				</button>
		</div>
	</div>
</div>
...
<div class="card">
	<div class="card-body">
			<div class="row" style="margin-bottom: 40px">
				<div class="col-sm-5">
					<h4 class="card-title mb-0"><i class="nav-icon fa fa-money"></i> Costing Details</h4>
				</div>
			</div>
			<div class="row"  style="margin-bottom: 20px">
				<div class="col-sm-12">
					<button class="btn btn-sm btn-secondary btn-action pull-left" onclick="costing_build(<?php echo $id ?>, 1)">
						<span class="miniaction"><img src="images/refresh.png"/>&nbsp;Refresh</span>						
					</button>
					
					<button class="btn btn-sm btn-secondary btn-action pull-left">
						<a class="link-action" href="<?php echo base_url() . "index.php/costing/prints/" . $id . "/print/0" ?>" target="blank"><img src="images/print.png">&nbsp;Print</a>&nbsp;						
					</button>
			        ...
			        if ($costing->locked == 'f' && $this->session->userdata('department') == 9) {
			            if (in_array('change_detail', $accessmenu)) {
			                ...
			                <button class="btn btn-sm btn-secondary btn-action pull-left">
								<span class="link-action"  onclick="costing_load_all_material(<?php echo $id . "," . $costing->modelid ?>, 2)" class = "miniaction" ><img src="images/load-material.png"/>&nbsp;Load All Material</span>&nbsp;						
							</button>
							
							<button class="btn btn-sm btn-secondary btn-action pull-left">
								<a class="link-action"  href="javascript:void(0)" onclick="costing_copyfrom(<?php echo $id ?>)"><img src="images/copy.png"/>&nbsp;Copy From Existing Costing</a>&nbsp;						
							</button>
			                
			                <button class="btn btn-sm btn-secondary btn-action pull-left">
								<span class="link-action"  onclick="costing_build(<?php echo $id ?>, 2)" class = "miniaction" ><img src="images/load-material.png"/>&nbsp;$&nbsp;Load Price</span>&nbsp;						
							</button>
							
			                <button class="btn btn-sm btn-secondary btn-action pull-left">
								<span class="link-action"  onclick="costing_lock(<?php echo $id ?>)" class = "miniaction"  style = "float: right;margin-right: 50px;"><img src = "images/lock.png"/>&nbsp;Lock</span>						
							</button>
		    ...
		    <div id="datacosting_detail" class="row">
				...
				<table class="table table-responsive-sm table-bordered table-striped table-hover table-outline mb-0">
			            ...
			            foreach ($costingcategory as $costingcategory) {
			                ...
			                <tr>                    
			                    ...
			                    <td colspan="12" bgcolor="#e9efe8" style="font-size: 11px;">  
			                        ...
			                        if ($costing->locked == 'f' && $this->session->userdata('department') == 9) {
			                            if (in_array('change_detail', $accessmenu)) {
			                                echo "<img src='images/bomadd.png' class='miniaction' onclick='costing_adddetail(" . $id . "," . $costingcategory->id . ",\"" . $costingcategory->name . "\" )'/>&nbsp;Add";
			            ...
			            <tr style="border-left: 1px solid #fff;line-height: 3;">
			            ...
			            foreach ($costingcategorynotdirect as $costingcategory) {
			                $costingdetail = $this->model_costing->selectCostingByHeaderidAndCategoryId($id, $costingcategory->id);
			                ?>
			                <tr style="background-color: #84d0ea">
			                    <td colspan="13" style="font-size: 11px;"><b><?php echo $costingcategory->name ?></td>
			                </tr>
			                <tr>
			                	<td colspan="13" bgcolor="#e9efe8" style="pull-left">
			                    	<?php
			                        if ($costing->locked == 'f' && $this->session->userdata('department') == 9) {
			                            if (in_array('change_detail', $accessmenu)) {
			                                echo "<img src='images/bomadd.png' class='miniaction' onclick='costing_adddetail(" . $id . "," . $costingcategory->id . ",\"" . $costingcategory->name . "\")'/>&nbsp;Add";
			...
			<div class="row" style="margin-bottom: 10px">
					<div class="col-sm-12">
					    ...
					    if ($this->session->userdata('department') == 9) {
					        if (in_array('change_detail', $accessmenu) || true) {
					            if ($flag == 2) {
					                echo "<button class='btn btn-md btn-primary pull-left' onclick='costing_updatematerialprice(" . $id . "," . $valid . ")' style='font-size: 11px;'>Update Material Price</button>";
					            } else {
// 					                if ($costing->locked == 'f') {
// 					                    if ($costing->isreviewed == 'f') {
					                        echo "<button class='btn btn-md btn-primary pull-left' onclick='costing_savefobprice(" . $id . ")' style='font-size: 11px;'>Save1</button>";
// 					                    } else {
					                        echo "<button class='btn btn-md btn-primary pull-left' onclick='costing_savefobprice(" . $id . ")' style='font-size: 11px;display:none;' id='costingsave'>Save2</button>";
					                        echo "<button class='btn btn-md btn-primary pull-left' onclick='costing_review()' style='font-size: 11px;' id='review'>Review</button>";
					                        echo "<button class='btn btn-md btn-primary pull-left' onclick='costing_ok()' style='font-size: 11px;display:none;' id='ok'>OK</button>";
					                        echo "<button class='btn btn-md btn-primary pull-left' onclick='costing_cancel()' style='font-size: 11px;display:none;' id='cancel'>Cancel</button>";
					                        echo "<button class='btn btn-md btn-primary pull-left' onclick='costing_review_fixed_profit_percentage()' id='review_fixed_profit_percentage' style='font-size: 11px;display:none;'>Fixed Profit Percentage</button>";
					                        echo "<button class='btn btn-md btn-primary pull-left' onclick='costing_review_fixed_sell_price()' id='review_fixed_sell_price' style='font-size: 11px;display:none;'>Fixed Sell Price</button>";
// 					                    }
// 					                }
					            }
					        }
